  ...
  for (const chunk of CHUNKS) {
    try {
      ...
      while ((match = regex.exec(text)) !== null) {
          if (match[1].includes("oktransit") || match[1].includes("oktransporte")) {
              urls.push(match[1]);
          }
      }
      ...
      // Also look for "apiGateway" patterns or "/v"
      if (text.includes("apiGateway")) {
          console.log(`\n'apiGateway' is in ${chunk}`);
          let pos = 0;
          while (true) {
              const idx = text.indexOf("apiGateway", pos);
              if (idx === -1) break;
