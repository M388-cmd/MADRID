...
async function main() {
  ...
  const patterns = [
    /https?:\/\/api-u\.oktransit\.app\/[^\s"']*/gi,
    /https?:\/\/api\.oktransit\.app\/[^\s"']*/gi,
    /\/v1\/madrid\/[^\s"']*/gi,
    /\/v1\/stops\/[^\s"']*/gi
  ];
  ...
  console.log("=== Found Endpoints & URL Patterns ===");
  console.log(Array.from(endpointsFound));
}
